...
/** `WordCount` experiment fixtures for the 'sysml-benchmark' bundle. */
@Configuration
class wordcount extends ApplicationContextAware {
  ...
  def `datagen.words`: FlinkJob = new FlinkJob(
    ...
    command =
      """
        |-v -c eu.stratosphere.benchmarks.systemml.datagen.flink.WordGenerator        \
        |${app.path.datagens}/sysml-benchmark-datagens-1.0-SNAPSHOT.jar        \
  ...
  def `wordcount.default`: ExperimentSuite = {
    val `wordcount.flink.default` = new FlinkExperiment(
      ...
      command =
        """
          |-v -c eu.stratosphere.benchmarks.systemml.flink.FlinkWC                      \
          |${app.path.apps}/sysml-benchmark-flink-jobs-1.0-SNAPSHOT.jar          \
          |${system.hadoop-2.path.input}/rubbish.txt                            \
    ...
    val `wordcount.spark.default` = new SparkExperiment(
      ...
      command =
        """
          |--class eu.stratosphere.benchmarks.systemml.spark.SparkWC                    \
          |${app.path.apps}/sysml-benchmark-spark-jobs-1.0-SNAPSHOT.jar          \
          |${system.hadoop-2.path.input}/rubbish.txt                            \
          |${system.hadoop-2.path.output}/wordcount
        """.stripMargin.trim,
      config  = ConfigFactory.parseString(""),
