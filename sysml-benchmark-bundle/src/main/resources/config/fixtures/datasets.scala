...
class datasets extends ApplicationContextAware {
  ...
  // provided
  // ---------------------------------------------------
  @Bean(name = Array("linreg.datagen.features.provided"))
  def `datagen.linreg.features.provided`: YarnJob = new YarnJob(
    command =
        s"""
           |jar $${app.path.apps}/SystemML.jar \\
           |org.apache.sysml.api.DMLScript \\
  ...
  @Bean(name = Array("linreg.datagen.features_split.provided"))
  def `datagen.linreg.features_split.provided`: YarnJob = new YarnJob(
    command =
        s"""
           |jar $${app.path.apps}/SystemML.jar \\
           |org.apache.sysml.api.DMLScript \\
  ...
  // provided
  // ---------------------------------------------------
  @Bean(name = Array("linreg.dataset.features.provided"))
  def `linreg.output.features.provided`: DataSet = new GeneratedDataSet(
    src = ctx.getBean("linreg.datagen.features.provided", classOf[YarnJob]),
    dst = "${system.hadoop-2.path.output}/linRegData.bin",
    fs  = ctx.getBean("hdfs-2.7.1-provided", classOf[HDFS2])
  )

  @Bean(name = Array("linreg.dataset.features_split.provided"))
  def `linreg.output.features_split.provided`: DataSet = new GeneratedDataSet(
    src = ctx.getBean("linreg.datagen.features_split.provided", classOf[YarnJob]),
    dst = "${system.hadoop-2.path.output}/linRegData.train.data.bin",
    fs  = ctx.getBean("hdfs-2.7.1-provided", classOf[HDFS2])
  )

  @Bean(name = Array("linreg.train.ds.output.provided"))
  def `linreg.train.ds.output.provided`: ExperimentOutput = new ExperimentOutput(
    path = "${system.hadoop-2.path.output}/betas.*",
    fs  = ctx.getBean("hdfs-2.7.1-provided", classOf[HDFS2])
  )
}
