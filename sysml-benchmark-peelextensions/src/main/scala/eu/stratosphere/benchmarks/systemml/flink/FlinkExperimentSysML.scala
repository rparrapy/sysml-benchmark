 ...
 */
package eu.stratosphere.benchmarks.systemml.flink
...
class FlinkExperimentSysML(
  ...
  override def run(id: Int, force: Boolean): Experiment.Run[Flink] = {
    new FlinkExperimentSysML.SingleJobRun(id, this, force)
  }

  def copy(name: String = name, config: Config = config) = {
    new FlinkExperimentSysML(command, systems, runner, runs, inputs, outputs, name, config)
  }
}

object FlinkExperimentSysML {
  ...
  class SingleJobRun(val id: Int, val exp: FlinkExperimentSysML, val force: Boolean) extends Experiment.SingleJobRun[Flink, State] {

    import eu.stratosphere.benchmarks.systemml.flink.FlinkExperimentSysML.StateProtocol._
    ...
    private def !(command: String, outFile: String, errFile: String) = {
      val expHdp = s"export HADOOP_HOME=${exp.config.getString(s"system.hadoop-2.path.home")}"
      val runExp = s"${exp.config.getString(s"system.${exp.runner.configKey}.path.home")}/bin/flink ${command.trim} > $outFile 2> $errFile"
      shell ! s"$expHdp ; $runExp"
    }
  }
