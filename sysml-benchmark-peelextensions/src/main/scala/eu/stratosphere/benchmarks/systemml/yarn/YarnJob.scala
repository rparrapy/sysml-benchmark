 ...
 */
package eu.stratosphere.benchmarks.systemml.yarn
...
/** Wrapper for a Job in Yarn.
  ...
  * `./bin/yarn` internally. More information on the available command-options can be found at
  ...
  *          <value>
  *            jar {app.path.someConfigVar}/my-datagen-job.jar                                            \
...
class YarnJob(command: String, runner: Yarn, timeout: Long) extends Job(command, runner, timeout) {
  ...
  private def !(command: String) = {
    shell ! s"${config.getString(s"system.${runner.configKey}.path.home")}/bin/yarn ${command.trim} >> $home/run.out 2>> $home/run.err"
  }
}
