...
/** Tear down a system. */
@Service("system:teardown")
class SystemTearDown extends Command {
  ...
  override def run(context: ApplicationContext) = {
    ...
    for (n <- graph.traverse(); if graph.descendants(sys).contains(n)) n match {
      case s: System if Lifespan.PROVIDED :: Lifespan.SUITE :: Lifespan.EXPERIMENT :: Lifespan.RUN :: Nil contains s.lifespan => s.tearDown()
      case _ => Unit
    }
  }
}
